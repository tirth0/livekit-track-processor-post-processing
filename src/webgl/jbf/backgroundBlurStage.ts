...
export function buildBackgroundBlurStage(
  ...
  try {
    ...
    cleanupCallbacks.push(blurPass.cleanUp);
    const blendPass = buildBlendPass(gl, positionBuffer, texCoordBuffer, canvas);
    cleanupCallbacks.push(blendPass.cleanUp);
...
function buildBlurPass(
  ...
  try {
    ...
    cleanupCallbacks.push(() => gl.deleteTexture(texture1));
    const texture2 = createTexture(gl, gl.RGBA8, outputWidth, outputHeight, gl.NEAREST, gl.LINEAR);
    cleanupCallbacks.push(() => gl.deleteTexture(texture2));

    const frameBuffer1 = gl.createFramebuffer();
    cleanupCallbacks.push(() => gl.deleteFramebuffer(frameBuffer1));
    gl.bindFramebuffer(gl.FRAMEBUFFER, frameBuffer1);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture1, 0);

    const frameBuffer2 = gl.createFramebuffer();
    cleanupCallbacks.push(() => gl.deleteFramebuffer(frameBuffer2));
