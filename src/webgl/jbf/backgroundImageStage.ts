...
export type BackgroundImageStage = {
  ...
  updateBlendMode(blendMode: JBFBlendMode): void;
  cleanUp(): void;
};
...
export function buildBackgroundImageStage(
  ...
  let pendingBackgroundImage: HTMLImageElement | null = null;

  try {
    addCleanup(() => gl.deleteTexture(backgroundTexture));

    const vertexShader = compileShader(gl, gl.VERTEX_SHADER, vertexShaderSource);
    addCleanup(() => gl.deleteShader(vertexShader));
    const fragmentShader = compileShader(gl, gl.FRAGMENT_SHADER, fragmentShaderSource);
    addCleanup(() => gl.deleteShader(fragmentShader));
    ...
    addCleanup(() => gl.deleteProgram(program));
    ...
    function updateBackgroundImage(nextBackgroundImage: HTMLImageElement) {
      ...
      backgroundTexture = createTexture(
        gl,
        gl.RGBA8,
        nextBackgroundImage.naturalWidth,
        nextBackgroundImage.naturalHeight,
        gl.LINEAR,
        gl.LINEAR,
      );
      gl.texSubImage2D(
      ...
      let xOffset = 0;
      let yOffset = 0;
      let backgroundWidth = nextBackgroundImage.naturalWidth;
      let backgroundHeight = nextBackgroundImage.naturalHeight;
      const backgroundRatio = backgroundWidth / backgroundHeight;
      if (backgroundRatio < outputRatio) {
        backgroundHeight = backgroundWidth / outputRatio;
        yOffset = (nextBackgroundImage.naturalHeight - backgroundHeight) / 2;
      } else {
        backgroundWidth = backgroundHeight * outputRatio;
        xOffset = (nextBackgroundImage.naturalWidth - backgroundWidth) / 2;
      }

      const xScale = backgroundWidth / nextBackgroundImage.naturalWidth;
      const yScale = backgroundHeight / nextBackgroundImage.naturalHeight;
      xOffset /= nextBackgroundImage.naturalWidth;
      yOffset /= nextBackgroundImage.naturalHeight;
    ...
    function updateBlendMode(blendMode: JBFBlendMode) {
      gl.useProgram(program);
    ...
    function cleanUp() {
      if (pendingBackgroundImage) {
        pendingBackgroundImage.onload = null;
        pendingBackgroundImage = null;
      }
